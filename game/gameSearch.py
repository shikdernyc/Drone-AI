...
def searchSolution(map,configuration,state,aiBaseName,tracep):
    ...
    if result:
        ...
        plan = list ( (a[0],{'showText': 'Executing {0} -> State {1}'.format(a[0],a[1])} )  for a in detailed_path)
...
def transformMap (map,configuration):
    ...
    agentPos = []
    agentName = configuration['agentType']
    position_rows = [r for r,v in enumerate(map) if agentName in [c[2]['agent'] for c in v]]
    for r in position_rows:
        position_cols = [c for c,v in enumerate(map[r]) if v[2]['agent']==agentName ]
        for c in position_cols:
            agentPos.append((r,c))
